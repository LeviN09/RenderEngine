...
void HeightmapRender::InitShared()
{
    ...
    for (uint64_t i = 0; i < corrected_res; ++i)
    {
        for (uint64_t j = 0; j < corrected_res; ++j)
        {
            ...
            PushToVerts({top_l.x, top_l.y, top_l.z,    top_l_norm.x, top_l_norm.y, top_l_norm.z,    top_l.y / 20.0f + 0.5f, top_l.y / 20.0f, 0.0f,    i       * percent, j       * percent});
            PushToVerts({bot_l.x, bot_l.y, bot_l.z,    bot_l_norm.x, bot_l_norm.y, bot_l_norm.z,    bot_l.y / 20.0f + 0.5f, bot_l.y / 20.0f, 0.0f,    i       * percent, (j + 1) * percent});
            PushToVerts({bot_r.x, bot_r.y, bot_r.z,    bot_r_norm.x, bot_r_norm.y, bot_r_norm.z,    bot_r.y / 20.0f + 0.5f, bot_r.y / 20.0f, 0.0f,    (i + 1) * percent, (j + 1) * percent});
            PushToVerts({top_r.x, top_r.y, top_r.z,    top_r_norm.x, top_r_norm.y, top_r_norm.z,    top_r.y / 20.0f + 0.5f, top_r.y / 20.0f, 0.0f,    (i + 1) * percent, j       * percent});
