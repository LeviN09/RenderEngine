...
{
    float diffDot = dot(norm, lightDir);
    diffDot *= 5.0f;
    diffDot = floor(diffDot);
    diffDot /= 5.0f;

    float dirDiffDot = dot(norm, dirLightDir);
    dirDiffDot *= 5.0f;
    dirDiffDot = floor(dirDiffDot);
    dirDiffDot /= 5.0f;
...
vec3 getSpecular(vec3 norm, vec3 lightDir)
{
    ...
    specDot *= 50.0f;
    specDot = floor(specDot);
    specDot /= 50.0f;

    float spec = pow(max(specDot, 0.0f), 32);

    float dirSpecDot = dot(viewDir, dirReflectDir);
    dirSpecDot *= 50.0f;
    dirSpecDot = floor(dirSpecDot);
    dirSpecDot /= 50.0f;

    float dirSpec = pow(max(dirSpecDot, 0.0f), 32);
